(function () {
  ...
  /* ============================================================
     AUTH GUARD
     Same pattern as billing.js — api.js must load first.
     Void-bill and payment/STK actions are backend-enforced per role
     (see billRoutes/paymentRoutes/mpesaRoutes), this just keeps
     non-admins off the page.
     ============================================================ */
  ...
  function renderTopbarName() {
    ...
    if (nameEl && sessionUser.first_name) nameEl.textContent = sessionUser.first_name;
  ...
  function renderBillBody(billId) {
    ...
    body.innerHTML = `
      ...
      ${!isVoid ? `
        <div class="bill-actions">
          ...
          <button class="btn btn-danger-outline btn-sm" data-action="void" data-id="${bill.id}">Void bill</button>
        </div>
      ` : ''}
    `;
    ...
    const voidBtn = body.querySelector('[data-action="void"]');
    if (voidBtn) voidBtn.addEventListener('click', () => voidBill(bill.id));
  }
  ...
  // without collapsing whatever else the admin has expanded.
  ...
  async function voidBill(billId) {
    ...
    try {
      await fetchMethod(`/bills/${billId}/void`, 'PUT', null, true);
      showToast('Bill voided');
      await refreshBillsAndKeepOpen(billId);
    } catch (err) {
      showToast(err.message || 'Could not void this bill');
    }
  }
  ...
  /* ============================================================
     STK PUSH MODAL — Safaricom themed (admin-initiated)
     ============================================================ */
